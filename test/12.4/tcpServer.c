...
//	fclose(sendf);
	while(1){
		...
		if(!strcmp(s_buf, "s\n")){
        	...
        	printf("USER input  %s\n", "Sound recording");
	        if((sendf = fopen("/home/seojeong/Desktop/systemprogramming/12.4/test.mp3", "rb+")) == NULL){
			perror("fopen: test.mp3");
               		exit(1);
		}
		...
		memset(buf, '\0', BUF_SIZ);
                ...
                if(recv(comm_socket, buf, BUF_SIZ, 0) != -1){
                        if((recvf = fopen("/home/seojeong/Desktop/systemprogramming/12.4/recv.mp3", "wb+")) == NULL){
                ...
                system("aplay /home/seojeong/Desktop/systemprogramming/12.4/recv.mp3");
                printf("YOU: %s\n", "Sound recording");
